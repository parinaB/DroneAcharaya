...
together. Everything downstream (`simulation/`, `backend/`, `ml/`,
`frontend/`, eventually Unreal) is built to speak these files. They are
**drafts** (`schema_version: 0.1-draft`, multiple `TBD`/`[TBD-*]` values) —
not yet the frozen contract the build plan calls for, but this is the shape
it's frozen into.
